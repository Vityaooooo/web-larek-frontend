...
* ```handleResponse(response: Response): Promise<object>Promise<object>``` - для обработки запроса с сервера
...
Является абстрактным классом и основой для создания всех компонентов пользовательского интерфейса, при помощи ```super()```, инициализировать контейнер компонента, который будет использован для создания компонента, предоставляет инструментарий для работы с DOM в дочерних элементах
...
Является абстрактным классом и основой для создания моделей данных для работы приложения, позволяет "под капотом", при помощи ```super()```, инициализировать данные наследника класса Model, реализует механизм уведомления всех подписчиков об изменении данных при помощи функции ```emitChanges```
...
* ```message: Message``` - сообщение об ошибки в данных для атрибута *order*
...
* ```getBasketCardId(): string[]``` - для получения id карточек из корзины для дальнейшего формирования заказа или проверки доступности кнопки добавления в корзину
